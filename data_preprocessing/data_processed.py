...
def main():
    ...
    # 处理命令行步骤控制参数的特殊逻辑
    # 如果用户在命令行明确指定了 --validation-only 或 --processing-only
    # 这些参数应该精确控制执行哪些步骤，而不是累加到配置文件的设置上
    if hasattr(args, 'dynamic_validation_only') and args.dynamic_validation_only:
        # 用户明确要求只验证，确保处理步骤不会因配置文件而启用
        # 除非用户同时也指定了 --processing-only
        if not (hasattr(args, 'dynamic_processing_only') and args.dynamic_processing_only):
            config.set('dynamic_processing_only', False)
    
    if hasattr(args, 'dynamic_processing_only') and args.dynamic_processing_only:
        # 用户明确要求只处理，确保验证步骤不会因配置文件而启用
        # 除非用户同时也指定了 --validation-only
        if not (hasattr(args, 'dynamic_validation_only') and args.dynamic_validation_only):
            config.set('dynamic_validation_only', False)
