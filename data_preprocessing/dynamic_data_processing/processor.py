...
class DynamicDataProcessor:
    ...
    def _define_columns_to_delete(self) -> List[str]:
        ...
        # 如果列删除功能未启用，返回空列表
        if not self.config.get('enable_column_deletion', False):
        ...
        column_type_mapping = {
            ...
            'inflammatory_biomarker': 'Inflammatory Biomarker',
            'vcn': 'VCN',
            'lymphocyte_subsets': 'Lymphocyte Subsets',
            'electrolytes': 'Electrolytes',
            'vital_signs': 'Vital Signs'
        }
        
        # 遍历所有配置的列类型
        for config_key, column_prefix in column_type_mapping.items():
            indices_to_delete = deletion_config.get(config_key, [])
            if indices_to_delete:
                # 根据索引构建完整列名
                columns_to_delete.extend([f"{column_prefix}{str(i).zfill(3)}" for i in indices_to_delete])
    ...
    def process_single_file(self, 
        ...
        try:
            ...
            # 列重新排序（可选功能）
            enable_reordering = self.config.get('enable_column_reordering', True)
            
            if enable_reordering:
                # 获取需要重新排序的列前缀
                prefixes_to_rename = self.config.get('column_prefixes_to_reorder', 
                ...
                # 如果禁用重新排序，记录在结果中
                result['reordering_disabled'] = True
    ...
    def process_all_files(self, 
        ...
        print(f"列重新排序: {'启用' if self.config.get('enable_column_reordering', True) else '禁用'}")
        ...
        if self.config.get('enable_column_deletion', False):
            ...
            column_type_display = {
                ...
                'inflammatory_biomarker': ('炎症标志物', 'Inflammatory Biomarker'),
                'vcn': ('病毒拷贝数', 'VCN'),
                'lymphocyte_subsets': ('淋巴细胞亚群', 'Lymphocyte Subsets'),
                'electrolytes': ('电解质', 'Electrolytes'),
                'vital_signs': ('生命体征', 'Vital Signs')
            }
            
            print(f"\n要删除的列配置:")
            has_deletions = False
            
            for config_key, (display_name, prefix) in column_type_display.items():
                indices = deletion_config.get(config_key, [])
                if indices:
                    has_deletions = True
                    column_names = [f'{prefix}{str(i).zfill(3)}' for i in indices]
                    print(f"- {display_name}列: {column_names}")
        ...
        # 显示重新排序配置
        if self.config.get('enable_column_reordering', True):
            prefixes = self.config.get('column_prefixes_to_reorder', [])
            if prefixes:
                print(f"\n列重新排序配置:")
                print(f"- 将对以下前缀的列重新编号: {prefixes}")
    ...
    def export_processing_report(self, output_path: str = None):
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"列删除功能: {'启用' if self.config.get('enable_column_deletion', False) else '禁用'}\n")
            f.write(f"列重新排序: {'启用' if self.config.get('enable_column_reordering', True) else '禁用'}\n\n")
            
            if self.config.get('enable_column_deletion', False):
                f.write("删除的列配置:\n")
                has_deletions = False
                
                for config_key, (display_name, prefix) in column_types.items():
                    indices = deletion_config.get(config_key, [])
                    if indices:
                        has_deletions = True
                        column_names = [f'{prefix}{str(i).zfill(3)}' for i in indices]
                        f.write(f"- {display_name}列: {column_names}\n")
                
                optional_columns = deletion_config.get('optional', [])
                if optional_columns:
                    has_deletions = True
                    f.write(f"- 可选列: {optional_columns}\n")
