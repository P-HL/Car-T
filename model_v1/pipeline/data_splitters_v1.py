...
class PatientLevelStratifiedSplitterWithCV:
    ...
    实现流程：
    1. 调用 base_splitter.split() 进行70/30划分
    2. 保存 train_indices_, test_indices_
    3. 调用 _group_stratified_kfold() 生成CV折叠
    4. 返回 (train_df, test_df, cv_folds_患者ID)
    
    类中定义的方法（4个）:
    __init__(self, test_size=0.3, n_folds=5, random_state=42, verbose=True)
    初始化分割器参数
    创建基础分割器 PatientLevelStratifiedSplitter 实例
    
    _group_stratified_kfold(self, df, group_col, label_col) (私有方法)
    实现患者级别的分层交叉验证
    确保同一患者数据不跨fold
    保持各fold中正负样本比例平衡
    
    split(self, df, label_col, patient_id_col="patient_id")
    执行主要的数据划分
    首先进行70/30主划分
    然后在训练集上创建交叉验证折叠
    返回训练集、测试集和CV折叠信息
    ...
    def _group_stratified_kfold(
        ...
        实现逻辑：
        数据打乱: 使用固定随机种子打乱数据
        正负样本分离: 将数据按标签分为正样本和负样本
        等分分组:
        将正样本等分成 n_folds 份
        将负样本等分成 n_folds 份
        构建折叠:
        第 i 折的验证集 = 第 i 份正样本 + 第 i 份负样本
        第 i 折的训练集 = 除验证集外的所有患者
